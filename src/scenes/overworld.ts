...
export const playerSpriteSheet = createImageElement("/player.png");
export const playerSkateSpriteSheet = createImageElement("/player-skate.png");
...
function getPlayerAnimation(): AnimationName {
  ...
  if (shouldAnimate) {
    ...
    if (isMovingFaster) return `run_${dir}`;
    return `walk_${dir}`;
  }
...
function updatePlayer(dt: number) {
  ...
  player.speed = movementSpeeds[getIsMovingFaster() ? "run" : "walk"];
